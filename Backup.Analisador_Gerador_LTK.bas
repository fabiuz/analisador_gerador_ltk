            ...
            SELECT CASE AS LONG CB.CTL
               ...
               CASE %IDC_TXT_ARQUIVO_ORIGEM, %IDC_TXT_PASTA_DESTINO_INSERT
                ...
                CASE %IDC_BTN_GERAR_LOTERIA
                    ...
                    ' Indica o diretório csv que foi processado.
                    ...
                    ' Vamos verificar se o diretório onde serão colocados os arquivos *.sql gerado existe.
                    ...
                    IF NOT ISFOLDER(strdiretorio_sql_gerado) THEN
                        strErro += "Diretorio '" + strDiretorio_sql_gerado + "', onde ficarão os arquivos *.sql gerados, não existe."
                    END IF


                    ' Vamos verificar se o diretório onde serão colocados os arquivos *.csv processados existe.
                    ...
                    IF NOT ISFOLDER(strdiretorio_csv_processado) THEN
                        strErro += IIF$(strErro <> "", $CRLF, "") + "Diretorio '" + strDiretorio_csv_processado + "', onde ficarão os arquivos *.csv processados, não existe."
                    END IF

                    LOCAL strArquivo_CSV AS STRING
                    CONTROL GET TEXT hJanela_Principal, %TXT_ARQUIVO_CSV TO strArquivo_CSV

                    ' Vamos verificar se o diretório onde serão lidos os arquivos *.csv e onde serão movidos os arquivos *.csv quando processados
                    ' não são iguais.
                    IF PATHSCAN$(PATH, strArquivo_csv) = PATHSCAN$(PATH, strdiretorio_csv_processado) THEN
                        strErro += IIF$(strErro <> "", $CRLF, "") + "Diretório onde serão lidos os arquivos *.csv e " + $CRLF
                        strErro += "diretorio onde serão movidos os arquivos *.csv após processados, são iguais."
                    END IF

                    ' Se o nome do arquivo fornecido pelo usuário é uma pasta, indicar um erro.
                    IF ISTRUE(ISFOLDER(strArquivo_csv)) THEN
                        strErro += IIF$(strErro <> "", $CRLF, "") + "'" + strArquivo_csv + "' não é um arquivo, é um diretório."
                    END IF

                    ' Se o arquivo fornecido pelo usuário, não termina em *.csv, indicar também um erro.
                    IF UCASE$(PATHNAME$(EXTN, strArquivo_csv)) <> ".CSV" THEN
                        strErro += IIF$(strErro <> "", $CRLF, "") + "'" + strArquivo_csv + "' não termina com a extensão '.csv'."
                    END IF

                    ' Se foi encontrado algum erro, strErro é um string não vazio.
                    IF strErro <> "" THEN
                        MSGBOX strErro, %MB_ICONERROR OR %MB_TASKMODAL
                        EXIT FUNCTION
                    END IF

                    ' Se o usuário fornece um filtro ou não, iremos utilizar o loop e utilizar a sentença 'DIR'
                    ' para verificar se há mais arquivos.
                    ...
                    ' Coloca a barra final, se não houver.
                    ...
                    ' Desativar então o controle, enquanto executarmos a função.
...
FUNCTION Validar_Campos_Loteria (BYVAL strArquivoOrigem AS STRING, BYREF strLinhas() AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    ' Vamos colocar no início da variável, o nome do arquivo.
    ' Na versão 5.0 deste arqui, colocamos o nome do arquivo no ínicio do string que indica
    ' erro, para ficar fácil identificar o arquivo.
    ...
    FOR uA = 2 TO uLinhas
        ...
        ' Incluído na versão 5.0
        ELSEIF strJogoTipo = "INTRALOT_LOTO_MINAS" AND uBolasQuantidade <> 6 THEN
            strErro += "No jogo 'INTRALOT_LOTO_MINAS' são sorteados 6 números, entretanto, no arquivo, " & _
                        "há somente " + FORMAT$(uBolasQuantidade) + " campos que correspondem às bolas."
            FUNCTION = -1
            EXIT FUNCTION
        ...
        ELSEIF strJogoTipo = "INTRALOT_KENO_MINAS" AND uBolasQuantidade <> 20 THEN
            strErro += "No jogo 'INTRALOT_KENO_MINAS' são sorteados 20 números, entretanto, no arquivo, " & _
                        "há somente " + FORMAT$(uBolasQuantidade) + " campos que correspondem às bolas."
            FUNCTION = -1
            EXIT FUNCTION
        END IF
...
FUNCTION Gerar_Insert_Where_Resultado_Loteria(BYVAL strArquivoOrigem AS STRING, BYVAL strPastaDestino AS STRING, BYREF strErro AS STRING, BYREF strSucesso AS STRING) AS LONG
    ...
    FOR uA = 2 TO uLinhas
        ...
        ' A variável uLimite, indica a quantidade de bolas a combinar.
        LOCAL uLimite AS LONG
...
INSERIR_COMBINACAO_LOTERIA:
    ...
    ' Vamos evitar que INTRALOT_KENO_MINAS fique muito grande, vamos limitar somente até 1 bola.
