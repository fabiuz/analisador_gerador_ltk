...
FUNCTION Combinacao(BYVAL strTexto AS STRING, BYVAL strDelimitador AS STRING, BYVAL strCombinacoes() AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    DIM objColecao AS ILINKLISTCOLLECTION
    LET objColecao = CLASS "LinkListCollection "
    ...
    uQuantidade_de_Campos = PARSECOUNT(strTexto, ANY FORMAT$(btDelimitador)
    IF uQuantidade_de_Campos = 1
...
GERAR_COMBINACAO:
    ...
    FOR lngA = 1 TO uLimite
        strBolas_Combinadas = strBolas_Combinadas & "_" & FORMAT$(strCampos(uIndice(lngA)), "00")
    NEXT
