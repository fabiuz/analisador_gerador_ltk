...
FUNCTION Validar_Campos_Loteria (BYVAL strArquivoOrigem AS STRING, BYREF strLinhas() AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    ' JOGO_TIPO;CONCURSO;DATA;BOLA1;BOLA2;...
    ...
    FOR uB = 4 TO uCampos_Quantidade
        ...
        IF TRIM$(strCampos(uB -3)) <> "BOLA" & FORMAT$(uB-3) THEN
...
FUNCTION Abrir_Arquivo_Para_Gravar_Loteria(BYVAL strJogo AS STRING, BYVAL strPastaDestino AS STRING, BYREF uInsert_Jogo_Bolas, _
                                            BYREF uInsert_Jogo_Combinacoes, BYREF uWhere_Jogo_Bolas, _
                                            BYREF uWhere_Jogo_Combinacoes) AS LONG
    ...
    TRY
        OPEN strJogo & "_JOGO_BOLAS_INSERT_" & strTempo & ".txt" FOR OUTPUT AS uInsert_Jogo_Bolas
        OPEN strJogo & "_JOGO_BOLAS_WHERE_" & strTempo & ".txt" FOR OUTPUT AS uWhere_Jogo_Bolas
        OPEN strJogo & "_JOGO_BOLAS_COMBINADAS_INSERT_" & strTempo & ".txt" FOR OUTPUT AS uInsert_Jogo_Combinacoes
        OPEN strJogo & "_JOGO_BOLAS_COMBINADAS_WHERE_" & strTempo & ".txt" FOR OUTPUT AS uWhere_Jogo_Combinacoes
    CATCH
        strErro = ERROR$(ERR)
        FUNCTION = -1
    END TRY
...
FUNCTION Gerar_Insert_Where_Resultado_Loteria(BYVAL strArquivoOrigem AS STRING, BYVAL strPastaDestino AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    strJogo = UCASE$(strLinhas(2))
    IF strJogo <> "QUINA" AND strJogo <> "MEGASENA" AND strJogo <> "LOTOFACIL" AND strJogo <> "LOTOMANIA" THEN
    ...
    IF Abrir_Arquivo_Para_Gravar_Loteria(strJogo, strPastaDestino, uInsert_Jogo_Bolas, uInsert_Jogo_Combinacoes, _
                                        uWhere_Jogo_Bolas, uWhere_Jogo_Combinacoes) = -1 THEN
        FUNCTION = -1
        EXIT FUNCTION
    END IF
    ...
    uQuantidade_de_Campos = PARSECOUNT(strLinha(1), ANY ";")
    ...
    FOR uA = 2 TO uLinhas
        '
        ' Carregar arranjo
        PARSE strLinha(uA), strCampos(), ANY ";"
        ...
        FOR uB = 1 TO uQuantidade_de_Bolas
            strBolas_Conjunto += "_" + FORMAT$(uBolas(uB))
        NEXT uB
...
INSERIR_COMBINACAO_LOTERIA:
    ...
    strInsert_Jogo_Bolas += "QT_DE_PARES, QT_DE_IMPARES, QT_QUADRANTE_1, QT_QUADRANTE_2, QT_QUADRANTE_3, QT_QUADRANTE_4) VALUES ("
    ...
    TRY
        ...
        PRINT #uInsert_Jogo_Combinacoes, strInsert_Bolas_Combinadas
