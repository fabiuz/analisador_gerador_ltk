...
FUNCTION Gerar_Insert_Where_Todas_Combinacoes(BYVAL strArquivoOrigem AS STRING, BYVAL strPastaDestino AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    ' Esta variável será definida quando entrarmos no loop
    ' Pois o loop será executado duas vezes
    DIM uVerificarCampos AS LONG
    ...
    ' Vamos ler a primeira linha e tentar detectar o tipo do jogo
    ...
    IF  strCampos(1) <> "QUINA" AND _
        strCampos(2) <> "MEGASENA" AND _
        strCampos(3) <> "LOTOFACIL" AND _
        strCampos(4) <> "LOTOMANIA" THEN
    ...
    ' 'COM' e 'NUMEROS'
    IF strCampos(2) <> "COM" THEN
        strErro = "Campo 2, da linha 1 inválido."
        FUNCTION = -1
        EXIT FUNCTION
    END IF

    IF strCampos(4) <> "NUMEROS" THEN
        strErro = "Campo 3, da linha 1 inválido."
        FUNCTION = -1
        EXIT FUNCTION
    END IF
    ...
    uCampos = PARSECOUNT(strLinha, ANY "_;")
    IF uCampos = 1 THEN
        strErro = "O arquivo não está no layout correto."
        FUNCTION = -1
        EXIT FUNCTION
    END IF
    ...
    ' Se chegarmos até aqui, o layout deve está correto iremos verificar
    DIM strJogo AS STRING

    ' Geralmente o layout no arquivo é:
    '   JOGO_COM_5_NUMEROS;1;2;3;4;5

    strJogo = PARSE$(strLinha, ANY "_;", 1)
    ' Vamos verificar se a linha é um jogo válido
    IF strJogo <> "QUINA" AND strJogo <> "MEGASENA" AND strJogo <> "LOTOFACIL" AND strJogo <> "LOTOMANIA" THEN
        strErro = "O campo 'JOGO': ' " & strJogo & "' inválido."
        FUNCTION = -1
        EXIT FUNCTION
    END IF

    ' Vamos verificar os campos 2 e 4
    DIM strCampoDois AS STRING
    DIM strCampoQuatro AS STRING
    strCampoDois = PARSE$(strLinha, ANY "_;", 2)
    strCampoTre
...
FUNCTION Gerar_Insert_Where_Todas_Combinacoes ALIAS "Gerar_Insert_Where_Todas_Combinacoes" (BYVAL strOrigem AS STRING, BYVAL strInsert AS STRING, BYVAL strWhere AS STRING, _
    ...
    ' Vamos verificar se o jogo solicitado, já está implementado
    ...
    SELECT CASE UCASE$(strJogo)
        ...
        CASE "QUINA"
            lngLimite = 5
        CASE ELSE
            strErro = "Jogo: '" & strJogo & "' não implementado"
            FUNCTION = -1
            EXIT FUNCTION
    END SELECT
    ...
    ' Se já está no fim do arquivo, sair então
    IF ISTRUE EOF(uOrigem) THEN
        strErro = "O arquivo está vazio!!!"
        CLOSE
        FUNCTION = -1
        EXIT FUNCTION
    END IF

    ' Vamos tentar abrir o arquivo para gravação
    DIM uInsert AS LONG
    uInsert = FREEFILE
    ...
    ' Variáveis para ser executadas no loop
    ...
    ' Vamos ler a primeira linha, é o cabeçalho
    ...
    DO WHILE ISFALSE EOF(uOrigem)
        ' Se chegarmos a esta linha, quer dizer, que o arquivo ainda não está no fim
        LINE INPUT #uOrigem, strLinha
        IF ERR <> 0 THEN
            GOTO manipulador_erro
        END IF
        ...
        IF PARSECOUNT(strLinha, ";") <> (lngLimite + 1) THEN
            strErro = "A linha: '" & strLinha & "' não tem " & FORMAT$(lngLimite + 1) & " campos."
            CLOSE
            FUNCTION = -1
            EXIT FUNCTION
        END IF

        ' Popular arranjo com os dados encontrados, converter-os para números
        DIM lngA AS LONG
