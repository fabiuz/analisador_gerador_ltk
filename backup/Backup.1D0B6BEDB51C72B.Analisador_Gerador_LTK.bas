    ...
    SELECT CASE strJogo
        ...
        CASE "LOTOMANIA"
            ...
            IF uBolasApostadas <> 50 THEN
                strErro = "Para o jogo 'LOTOMANIA' você deve apostar 50 números."
                FUNCTION = -1
                EXIT FUNCTION
            END IF
        END SELECT
            ...
            SELECT CASE AS LONG CB.CTL
                ...
                CASE %BTN_TESTE
                    IF CB.CTLMSG = %BN_CLICKED THEN
                        ...
                        CONTROL GET TEXT hJanela_Principal, %TXT_TESTE TO strTeste
                        IF strTeste = "" THEN
                            MSGBOX "Você deve fornecer um valor.", %MB_ICONSTOP OR %MB_APPLMODAL
                            EXIT FUNCTION
                        END IF
                    END IF
                    ...
                    ' Vamos apagar o listBox
                    LISTBOX RESET hJanela_Principal, %LST_TESTE

                    DIM strCombinacao() AS STRING
                    IF Permutacao(strTeste, ";", strCombinacao(), strErro) = -1 THEN
                        MSGBOX "Um erro ocorreu: " & strErro, %MB_ICONSTOP OR %MB_APPLMODAL
                        EXIT FUNCTION
                    END IF

                    LOCAL uA, uContador AS LONG
                    uContador = UBOUND(strCombinacao())

                    FOR uA = 1 TO uContador
                        LISTBOX ADD hJanela_Principal, %LST_TESTE, strCombinacao(uA)
                    NEXT uA
...
FUNCTION Validar_Campos_Loteria (BYVAL strArquivoOrigem AS STRING, BYREF strLinhas() AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    ' O campo 1, da linha 2, indica qual é o jogo, que está sendo processado.
    ' Então, todas as linhas seguintes, no campo 1, deve ter o mesmo jogo.
    ...
    IF  strJogoTipo <> "QUINA" AND strJogoTipo <> "MEGASENA" AND _
        ...
        strJogoTipo <> "INTRALOT_LOTOMINAS" THEN
    ...
    ' A quantidade de bolas é definido pela quantidade de campos menos 3.
    ' Pois, os três primeiros campos, indica o:
    ...
    FOR uA = 2 TO uLinhas
        ...
        ' A data deve está neste formato: dd/mm/yyyy ou d/m/yyyy, exemplo: 1/1/2015 ou 01/1/2015 ou 1/01/2015 ou 01/01/2015
        ' Verificar se contém os caracteres válidos.
        ' O terceiro campo indica a data

        IF VERIFY(strCampos(3), "0123456789/") <> 0 THEN
            strErro = "Erro, linha " + FORMAT$(uA) + ", campo 3, formato de data inválido: '" + strCampos(3) + "'"
            FUNCTION = -1
            EXIT FUNCTION
        END IF
        ...
        ' Vamos verificar se há somente três campos
        ...
        ' Se chegamos até aqui, quer dizer que há 3 campos, vamos verificar se os campos de data e mês estão dentro
        ' da faixa válida.
        DIM uData_Dia AS LONG
        uData_Dia = VAL(strData(1))

        IF uData_Dia < 1 OR uData_Dia > 31 THEN
            strErro  = "Erro, linha " + FORMAT$(uA) + ", campo 3, formato de data inválido: '" + strCampos(3) + "'." + $CRLF
            strErro += "O intervalo válido é maior ou igual a 1 e menor ou igual a 31."
            FUNCTION = -1
            EXIT FUNCTION
        END IF
        ...
        ' Se fora da faixa válida para o mês, indicar um erro.
        IF uData_Mes < 1 OR uData_Mes > 12 THEN
            strErro  = "Erro, linha " + FORMAT$(uA) + ", campo 3, formato de data inválido: '" + strCampos(3) + "'." + $CRLF
            strErro += "O intervalo válido para o mês deve estar entre 1 e 12."
            FUNCTION = -1
            EXIT FUNCTION
        END IF

        ' Cada mês, excetuando fevereiro, termina em 30 ou 31.
        ' Para o mês de fevereiro deveremos ver se é bissexto ou não para considerar, se é 29 ou 28.

        ' Se o dia termina em 31, deveremos verificar se o mês é
        ' Janeiro, Março, Maio, Julho, Agosto, Outubro, Dezembro, se não é, é um erro.
        IF uData_Dia = 31 THEN
            SELECT CASE AS LONG uData_Mes
                ...
                CASE ELSE
                    strErro  = "Erro, linha " + FORMAT$(uA) + ", campo 3, formato de data inválido: '" + strCampos(3) + "'." + $CRLF
                    strErro += "Dia 31 é inválido para o mês: " & FORMAT$(uData_Mes) & "."
                    FUNCTION = -1
                    EXIT FUNCTION
            END SELECT

        ' Devemos colocar os operandos do operador 'OR', entre parênteses
        ' Por que a precedência do operador 'OR' é menor que do operador 'AND'

        ELSEIF uData_Dia = 29 AND uData_Mes = 2 THEN
            ' Vamos verificar se o ano é bissexto, se não for, uData_Dia deve ser diferente de 29.
            ' Um número é bissexto, se for
            ' Divisível por 400;
            ...
            ' Se o ano não é bissexto, então o dia 29 não pode existir.
            IF ISFALSE((uData_Ano MOD 4 = 0 AND uData_Ano MOD 100 <> 0) OR uData_Ano MOD 400 = 0) THEN
                strErro  = "Erro, linha " + FORMAT$(uA) + ", campo 3, formato de data inválido: '" + strCampos(3) + "'." + $CRLF
                strErro += "Dia 29 para o mês de fevereiro somente é válido para ano bissexto, entretanto, " + $CRLF
                strErro += "o ano " + FORMAT$(uData_Ano) + " não é bissexto."
                FUNCTION = -1
                EXIT FUNCTION
            END IF
        END IF
        ...
        ' **************************** VERIFICAR SE HÁ BOLAS REPETIDAS ********************************
        '
        IF uA = 10 THEN
            '
        END IF
        ...
        FOR uX = 3 TO uCampos_Quantidade
            FOR uY = uX + 1 TO uCampos_Quantidade
                ' Se jogo é DUPLASENA, haverá 12 bolas, onde as seis primeiras correspondem ao primeiro sorteio
                ' e, as seis últimas bolas, correspondem ao segundo sorteio.
                ' então, não podemos comparar o primeiro sorteio com o segundo.
                ' O primeiro sorteio começa no índice 3 e vai até 8.
                ' O segundo sorteio começa em 9 e vai até 14.
                ' Então, se uX é menor ou igual a 8, estamos no primeiro sorteio, então
                ' não devemos comparar com o segundo sorteio que começa em uY = 9
                IF uA = 10 THEN
                    '
                END IF
                ...
                IF strCampos(uX) = strCampos(uY) THEN
                    ...
                    strErro += "Não pode haver bolas idênticas."
                    FUNCTION = -1
                    EXIT FUNCTION
                END IF
            NEXT uY
        ...
        ELSEIF strJogoTipo = "INTRALOT_MINAS_5" AND uBolasQuantidade <> 5 THEN
            strErro = "No jogo 'INTRALOT_MINAS_5' são sorteados 5 números, entretanto, no arquivo, " & _
                        " há somente " + FORMAT$(uBolasQuantidade) + " campos que correspondem às bolas."
            FUNCTION = -1
            EXIT FUNCTION
        END IF
...
#IF 0
    ' Vamos verificar se não há números duplicados em uma mesma linha
    ' A quantidade de bolas é igual ao número de campos menos 3, mas por que 3, por que
    ' o primeiro campo indica o tipo do jogo, por exemplo, 'QUINA', o segundo campo
    FOR uA = 2 TO uLinhas
        ' As bolas sempre começam no campo 3 em diante.
         PARSE UCASE$(strLinhas(uA)), strCampos(), ANY ";"
...
'
'   Esta função retorna todas as combinações possíveis para um conjunto
'
' Parâmetros:
'   strTexto:           Um string onde os elementos que fazem parte do conjunto é separado por um delimitador, por exemplo:
'                       strTexto --> 1;2;3;4;5
'   strDelimitador:     O caractere delimitador a ser o separador
'   strCombinacoes():   Retorna um arranjo, indicando todas as combinações geradas.
'   strErro:            Se ocorrer algum erro, indicar nesta variável.
'
' Obs.:                 Na função, por exemplo, o conjunto, 1;2;3;4, retornaria, alguns exemplos, 1;2, 1;3, 1;2;3, 2;3, 2;3;4.
'                       Ou seja, a função retorna somente o conjunto que não se repetem. Conclui-se:
'                       Na função: 2;3 e 3;2 são a mesma coisa.
...
FUNCTION Combinacao(BYVAL strTexto AS STRING, BYVAL strDelimitador AS STRING, BYREF strCombinacoes() AS STRING, BYREF strErro AS STRING) AS LONG
    ' Vamos verificar se os argumentos estão corretos.
    IF strTexto = "" THEN
        strErro = "Erro: 'strTexto' está com o conteúdo vazio."
        FUNCTION = -1
        EXIT FUNCTION
    END IF
    '
    ...
    IF ISFALSE(ISOBJECT(combinados_Colecao)) THEN
        strErro = "Erro: Não foi possível criar o objecto ILinkListCollection"
        FUNCTION = -1
        EXIT FUNCTION
    END IF
  ...
  ' Vamos executar todas as combinações possíveis.
    ...
    ' Vamos verificar quantas combinaçoes foram criados
...
FUNCTION Permutacao(BYVAL strTexto AS STRING, BYVAL strDelimitador AS STRING, BYREF strCombinacoes() AS STRING, BYREF strErro AS STRING) AS LONG
    ' Vamos verificar se os argumentos estão corretos.
    IF strTexto = "" THEN
        strErro = "Erro: 'strTexto' está com o conteúdo vazio."
        FUNCTION = -1
  ...
  ' Vamos executar todas as permutações possíveis.
    FOR uColuna1 = 1 TO uQuantidade_de_Campos
        'uIndice(1) = uColuna1
        uLimite = 1
        GOSUB GERAR_COMBINACAO
        '
        FOR uColuna2 = 1 TO uQuantidade_de_Campos
            'uIndice(2) = uColuna2
            uLimite = 2
            GOSUB GERAR_COMBINACAO
            '
            FOR uColuna3 = uColuna2 TO uQuantidade_de_Campos
                'uIndice(3) = uColuna3
                uLimite = 3
                GOSUB GERAR_COMBINACAO
                '
                FOR uColuna4 = uColuna3 TO uQuantidade_de_Campos
                    'uIndice(4) = uColuna4
                    uLimite = 4
                    GOSUB GERAR_COMBINACAO
                    '
                    FOR uColuna5 = 1 TO uQuantidade_de_Campos
                        uIndice(5) = uColuna5
                        uLimite = 5
                        GOSUB GERAR_COMBINACAO
                        '
                        FOR uColuna6 = 1 TO uQuantidade_de_Campos
                            uIndice(6) = uColuna6
                            uLimite = 6
                            GOSUB GERAR_COMBINACAO
                            '                              '
                            FOR uColuna7 = 1 TO uQuantidade_de_Campos
                                uIndice(7) = uColuna7
                                uLimite = 7
                                GOSUB GERAR_COMBINACAO
                                '
                                FOR uColuna8 = 1 TO uQuantidade_de_Campos
                                    uIndice(8) = uColuna8
                                    uLimite = 8
                                    GOSUB GERAR_COMBINACAO
                                    '                                         '
                                    FOR uColuna9 = 1 TO uQuantidade_de_Campos
                                        uIndice(9) = uColuna9
                                        uLimite = 9
                                        GOSUB GERAR_COMBINACAO
                                        '
                                        FOR uColuna10 = 1 TO uQuantidade_de_Campos
                                            uIndice(10) = uColuna10
                                            uLimite = 10
                                            GOSUB GERAR_COMBINACAO
                                            '
                                            FOR uColuna11 = 1 TO uQuantidade_de_Campos
                                                uIndice(11) = uColuna11
                                                uLimite = 11
                                                GOSUB GERAR_COMBINACAO
                                                '
                                                FOR uColuna12 = 1 TO uQuantidade_de_Campos
                                                    uIndice(12) = uColuna12
                                                    uLimite = 12
                                                    GOSUB GERAR_COMBINACAO
                                                    '
                                                    FOR uColuna13 = 1 TO uQuantidade_de_Campos
                                                        uIndice(13) = uColuna13
                                                        uLimite = 13
                                                        GOSUB GERAR_COMBINACAO
                                                        '
                                                        FOR uColuna14 = 1 TO uQuantidade_de_Campos
                                                            uIndice(14) = uColuna14
                                                            uLimite = 14
                                                            GOSUB GERAR_COMBINACAO
                                                            '
                                                            FOR uColuna15 = 1 TO uQuantidade_de_Campos
                                                                uIndice(15) = uColuna15
                                                                uLimite = 15
                                                                GOSUB GERAR_COMBINACAO
                                                                '
...
GERAR_COMBINACAO:
    ...
    FOR lngA = 1 TO uLimite
        'strCampos_Combinados = strCampos_Combinados & "_" & strCampos(uIndice(lngA))
        strCampos_Combinados = strCampos_Combinados & "_" & strCampos(lngA)
    NEXT
    combinados_Colecao.add(strCampos_Combinados)
...
FUNCTION Gerar_Insert_Where_Resultado_Loteria(BYVAL strArquivoOrigem AS STRING, BYVAL strPastaDestino AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    IF  strJogo <> "QUINA" AND strJogo <> "MEGASENA" AND strJogo <> "LOTOFACIL" AND strJogo <> "LOTOMANIA" AND _
        strJogo <> "INTRALOT_MINAS_5" AND strJogo <> "INTRALOT_LOTO_MINAS_5" AND strJogo <> "DUPLASENA" THEN
...
INSERIR_COMBINACAO_LOTERIA:
    ...
    IF strCampos(1) = "LOTOFACIL" AND uLimite > 3 THEN
        RETURN
    END IF

    ' Também evitaremos que LOTOMANIA, fique muito grande, colocaremos o limite de até 3.
    IF strCampos(1) = "LOTOMANIA" AND uLimite > 3 THEN
        RETURN
    END IF
    ...
    IF ((quantidade_de_combinacoes MOD 229370 = 0) AND TRIM$(strCampos(1)) = "LOTOFACIL") OR _
        ...
        ' Acima como o limite é de até 3, é possível gerar 1350 combinações.
        ' então, 337500 é igual a 250 * 1350
    ...
    ' A variáve abaixo serve para indicar quais os campos na tabela devemos inserir o valor '1'
    ' Se por exemplo, a combinação for dos números _05_07_12
    ' A variável terá este valor: ', num_5, num_7, num_12', pois assim, estaremos indicando quais campos
    ' serão atualizados.
    ...
    ' Abaixo, indicaremos quais os campos que serão atualizados.
    strInsert_Jogo_Bolas += strNumeros_Insert

    ' Devemos fechar os campos com ')' parênteses.
    ...
    strInsert_Jogo_Bolas += "'" & FORMAT$(uQuantidade_de_Bolas)  & "'"      ' Observe que nesta linha não há vírgula no final, pois, a vírgula está inserida
                                                                            ' na variável 'strNumeros_Insert'

    ' Aqui, iremos repetir vários '1' conforme a o valor de 'uLimite'.
    strInsert_Jogo_Bolas += REPEAT$(uLimite, ", '1'")

    ' Devemos, inserir o parênteses referente ao final do insert ')' + um CARRIAGE RETURN + LINEFEED
    strInsert_Jogo_Bolas += "); "  + $CRLF
...
#PBFORMS BEGIN DIALOG %IDD_DIALOG1->->
    ...
    DIALOG NEW hParent, "Analisador e Gerador - LTK", 278, 92, 563, 410, _
    ...
    CONTROL ADD TEXTBOX,  hDlg, %TXT_TESTE, "", 285, 190, 270, 13
    CONTROL ADD BUTTON,   hDlg, %BTN_TESTE, "&Testar", 505, 205, 50, 15
    CONTROL ADD LISTBOX,  hDlg, %LST_TESTE, , 285, 205, 140, 170
    CONTROL ADD LABEL,    hDlg, %LBL_TESTE, "", 285, 380, 270, 25, %WS_CHILD _
        OR %WS_VISIBLE OR %SS_CENTER OR %SS_SUNKEN, %WS_EX_LEFT OR _
        %WS_EX_LTRREADING
    CONTROL ADD BUTTON,   hDlg, %BTN_PARAR_TESTE, "&Parar", 505, 360, 50, 15
    CONTROL ADD BUTTON,   hDlg, %BTN_PAUSAR_TESTE, "Pau&sar", 505, 340, 50, _
        15
#PBFORMS END DIALOG
