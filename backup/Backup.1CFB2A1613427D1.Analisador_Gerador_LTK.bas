...
FUNCTION Validar_Campos_Loteria (BYVAL strArquivoOrigem AS STRING, BYREF strLinhas() AS STRING, BYREF strErro AS STRING) AS LONG
    ...
    FOR uB = 4 TO uCampos_Quantidade
        ...
        IF TRIM$(strCampos(uB)) <> "BOLA" & FORMAT$(uB-3) THEN
            strErro = "Erro, linha 1, Campo " & FORMAT$(uB-3) & " deve ter a palavra 'BOLA" & FORMAT$(uB-3) & "'."
            FUNCTION = -1
            EXIT FUNCTION
        END IF
    NEXT uB
...
FUNCTION Abrir_Arquivo_Para_Gravar_Loteria(BYVAL strJogo AS STRING, BYVAL strPastaDestino AS STRING, BYREF uInsert_Jogo_Bolas AS LONG, _
    ...
    strInsert_Jogo_Bolas = strPastaDestino & strJogo & "_JOGO_BOLAS_INSERT_" & strTempo & ".txt"
    strWhere_Jogo_Bolas = strPastaDestino & strJogo & "_JOGO_BOLAS_WHERE_" & strTempo & ".txt"
    strInsert_Jogo_Combinacoes = strPastaDestino & strJogo &  "_JOGO_BOLAS_COMBINADAS_INSERT_" & strTempo & ".txt"
    strWhere_Jogo_Combinacoes = strPastaDestino & strJogo &  "_JOGO_BOLAS_COMBINADAS_WHERE_" & strTempo & ".txt"
